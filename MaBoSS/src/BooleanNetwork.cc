...
const bool backward_istate = true;
...
NodeState Node::getIState( Network* network) const
{
  if (!istate_set) {
#if 1
...
void IStateGroup::initStates(Network* network, NetworkState& initial_state)
{
  ...
  while (istate_group_iter != istate_group_end) {
	...
	const Node* node = (*nodes)[nn++];
      double rand = network->getRandomGenerator()->generate();
	  //std::cerr << (*state_value_iter) << " " << rand << '\n';	
	initial_state.setNodeState(node, (rand <= (*state_value_iter)) ? true : false);
